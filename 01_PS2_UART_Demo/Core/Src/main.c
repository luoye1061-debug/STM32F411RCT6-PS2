...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
	AX_DELAY_Init();// 延时函数初始化
	AX_PS2_Init();// ps2手柄引脚电平初始化
	printf("\r\n  PS2无线手柄HAL库例程  \r\n");
  /* USER CODE END 2 */
  ...
  while (1)
  {
		AX_PS2_ScanKey(&my_joystick);// 手柄键值扫描
		// 打印手柄键值ֵ
		printf("MODE:%2x BTN1:%2x BTN2:%2x RJOY_LR:%2x RJOY_UD:%2x LJOY_LR:%2x LJOY_UD:%2x\r\n",
            ...
            my_joystick.RJoy_LR, my_joystick.RJoy_UD,
            my_joystick.LJoy_LR, my_joystick.LJoy_UD); //
